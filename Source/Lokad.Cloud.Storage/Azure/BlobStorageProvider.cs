    ...
    public class BlobStorageProvider : IBlobStorageProvider
    {
        ...
        bool PutBlob(string containerName, string blobName, object item, Type type, bool overwrite, string expectedEtag, out string outEtag, IDataSerializer serializer = null)
        {
            using (var stream = new MemoryStream())
            {
                ...
                stream.Seek(0, SeekOrigin.Begin);
                return PutBlobStream(containerName, blobName, stream, overwrite, expectedEtag, out outEtag);
            }
        }
        ...
        public Task<string> PutBlobAsync(string containerName, string blobName, object item, Type type, bool overwrite, string expectedEtag, CancellationToken cancellationToken, IDataSerializer serializer = null)
        {
            ...
            (serializer ?? _defaultSerializer).Serialize(item, stream, type);
            stream.Seek(0, SeekOrigin.Begin);
            return PutBlobStreamAsync(containerName, blobName, stream, overwrite, expectedEtag, cancellationToken).Then(etag =>
            {
                stream.Dispose();
                return etag;
            });
